...
(function (window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    getLabel_: function (el) {
      var label = 'label[for="' + el.getAttribute('id') + '"]';
      var labelEl = this.el_[this.current_].querySelectorAll(label); // check for multiple labels with identical 'for' attributes
    ...
    /** @return void */
    onInput_: function (ev) {
      ev.target.parentNode.classList[this.addRemove_(ev.target.value.length)](this.prefixed_('is-active'));
    },
    ...
    resetFields_: function () {
      ...
      for (var i = 0; i < fields.length; ++i) {
        fields[i].parentNode.classList[self.addRemove_(fields[i].tagName === 'SELECT' && fields[i].value !== '')](self.prefixed_('is-active'));
      }
    },
